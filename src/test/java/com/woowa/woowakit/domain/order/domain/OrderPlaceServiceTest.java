...
class OrderPlaceServiceTest {
	...
	private Coupon saveCoupon(
		...
		CouponFrame couponFrame = saveCouponFrame(couponTarget, couponType, discount);

		return couponRepository.save(couponFrame.makeCoupon(memberId, now));
	}

	private CouponFrame saveCouponFrame(
		final CouponTarget couponTarget,
		final CouponType couponType,
		final int discount
	) {
